...
using namespace std;
template <typename t>
class CustomHashing : public hashFunctions
{
private:
    int N;
    int collisions;
...
public:
    CustomHashing(int N = 0, int hashFunction = 1, int C1 = 23, int C2 = 29)
    {
    ...
    bool insert(string key, t value)
    {
        ...
        {
            index = (index + i * auxHash(key) * C1 + i * i * C2) % N;
            i++;
            probes++;
            // collisions++;
        }
    ...
    void reHash(bool flag)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            if (table[i].first != "")
            {
                ...
                {
                    index = (index + j * auxHash(table[i].first) * C1 + j * j * C2) % newN;
                    j++;
                    newCollisions++;
                }
